...
print("Initializing On-Chain Risk Monitoring Pipeline...")
...
data = {
    ...
    "Sender": [f"0x{''.join(np.random.choice(list('0123456789abcdef'), 40))}" for _ in range(n_transactions)],
...
# 2. Data Cleaning & Statistical Risk Backtesting (Z-Score Anomaly Detection)
mean_val = df["Value_ETH"].mean()
std_val = df["Value_ETH"].std()

# Flag transactions exceeding 3 standard deviations from the mean (statistical anomaly)
...
print(f"[SUCCESS] Analyzed {len(df)} transactions.")
print(f"-> Total Volume Tracked: {total_volume:.2f} ETH")
print(f"-> High-Risk Anomalies Flagged: {anomaly_count}")

# 3. Build Interactive Visual Dashboard using Plotly (with explicit specs for the table)
fig = make_subplots(
    rows=2, cols=2,
...
fig.update_layout(
    title_text="<b>On-Chain Risk & Anomaly Monitoring Dashboard</b>",
